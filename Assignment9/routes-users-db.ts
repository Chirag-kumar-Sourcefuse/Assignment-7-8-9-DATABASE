...
router.get("/", (req, res) => {
    pool.query("select m.id,m.firstname, m.middlename, m.lastname,m.email,m.phoneno,m.address,c.customer_name, r.name AS ROLE,m.createdon,m.modifiedon from mydata m JOIN customer c ON m.id=c.customer_id JOIN role r ON r.role_key=m.role_key;").then((userData) => {
...
router.put('/:Id',(req,res)=>{
 ...
 const mdate=`${new Date().toLocaleString()}`;
    pool.query(
        'UPDATE mydata SET firstname = $1,middlename=$2, lastname=$3, email=$4,phoneno=$5,address=$6,role_key=$7,modifiedon=$9 WHERE id=$8',
        [firstname,middlename,lastname,email,phoneno,address,role_key,id,mdate],
        (error, results) => {
          if (error) {
            throw error
          }

...
router.get("/", function (req, res) {
    db_1.pool.query("select m.id,m.firstname, m.middlename, m.lastname,m.email,m.phoneno,m.address,c.customer_name, r.name AS ROLE,m.createdon,m.modifiedon from mydata m JOIN customer c ON m.id=c.customer_id JOIN role r ON r.role_key=m.role_key;").then(function (userData) {
...
router.put('/:Id', function (req, res) {
    ...
    db_1.pool.query('UPDATE mydata SET firstname = $1,middlename=$2, lastname=$3, email=$4,phoneno=$5,address=$6,role_key=$7,modifiedon=$9 WHERE id=$8', [firstname, middlename, lastname, email, phoneno, address, role_key, id, mdate], function (error, results) {
        if (error) {
            throw error;
        }
